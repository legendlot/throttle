...
export class ConnectorWorkflow extends WorkflowEntrypoint {
  async run(event, step) {
    ...
    // ── FBA inventory HISTORY backfill: REMOVED FROM THE WORKFLOW (S289) ───────────────────────
    // A `backfill_fba_ledger` trigger used to walk 30-day windows in one run. It could not work:
    // SP-API createReport is ~1 req/60s with a ~15 burst, so windows 0-8 drained the bucket and
    // every remaining window failed 429 — retries included, since a 60s retry delay IS the refill
    // rate. Failed calls still consume quota, so restarting immediately made recovery slower.
    //
    // Superseded by `fbaLedgerBackfillTick()`, which does exactly ONE window per hourly cron tick
    // and is structurally incapable of hitting the limit. Deleted rather than left callable: it
    // carried a SECOND copy of the ledger parse (the quoted-fields / MM-DD-YYYY / SELLABLE-only
    // rules), so the two could silently drift, and it is precisely the looping shape the cron
    // version warns against. Use `runLedgerBackfillTickNow` (super-admin) to nudge it by hand.
