...
async function runGate(env, { profileId, channel, purpose, to, wa, isTest }) {
  ...
  if (channel === 'whatsapp') {
    ...
    // 'media' (agent attachment, S245) and 'list' (bot menu, S355) are session messages exactly
    // like the other two. ⚠️ AN UNLISTED MODE IS NOT GATED — every session mode must be listed
    // here, or it leaves the window check entirely and only the adapter stands between it and a
    // window-closed send.
    if (wa && (wa.mode === 'text' || wa.mode === 'interactive' || wa.mode === 'media' || wa.mode === 'list') && wa.window_open !== true)
