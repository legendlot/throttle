...
// GET /unsubscribe?token= — append opted_out consent + emit event. Returns HTML.
async function handleUnsubscribe(env, token) {
  ...
  await A.sbComms('/rest/v1/consent', env, {
    method: 'POST',
    body: JSON.stringify({
      profile_id: row.profile_id, channel: row.channel, purpose: 'marketing',
      state: 'opted_out', source: 'unsubscribe_link', unsubscribe_token: token,
    }),
  });
  await A.sbComms('/rest/v1/events', env, {
    method: 'POST',
    body: JSON.stringify({
      profile_id: row.profile_id, name: 'opted_out', source: 'unsubscribe_link',
      properties: { channel: row.channel, purpose: 'marketing' },
    }),
  });
  return { html: page("You've been unsubscribed from marketing emails. You'll still get essential order updates."), status: 200 };
}
...
<body><div class="card"><div class="bar"></div><h1>Legend of Toys</h1><p>${msg}</p></div></body></html>`;
}
