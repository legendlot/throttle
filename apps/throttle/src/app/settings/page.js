    ...
    return (
      <Layout>
        <div style={{ maxWidth: 512, margin: '0 auto', paddingTop: 80, paddingBottom: 80, textAlign: 'center' }}>
          <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--t3)' }}>Admin access required.</p>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <div style={{ maxWidth: 896, margin: '0 auto' }}>
        ...
        <p style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--t3)', marginBottom: 24 }}>Manage team roles and disciplines</p>

        {!loaded ? (
          <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--t3)' }}>Loading team...</p>
        ) : loadError ? (
          <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: '#DE2A2A' }}>{loadError}</p>
        ) : (
          <div>
            {error && (
              <div style={{ background: 'rgba(222,42,42,0.08)', border: '1px solid rgba(222,42,42,0.3)', borderRadius: 6, padding: '10px 14px', marginBottom: 16 }}>
                <p style={{ color: 'var(--red)', fontFamily: 'var(--sans)', fontSize: 12 }}>{error}</p>
              </div>
            )}
            {(() => {
              const GROUP_ORDER = [
              ...
              const thStyle = { fontFamily: 'var(--sans)', fontSize: 11, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--t3)', fontWeight: 600, padding: '9px 16px', textAlign: 'left', borderBottom: '1px solid var(--b1)' };
                ...
                return (
                  <div key={group.key} style={{ marginBottom: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      <span style={{ fontFamily: 'var(--sans)', fontSize: 12, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--t3)', fontWeight: 600 }}>{group.label}</span>
                      <span style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--t3)', background: 'var(--s2)', border: '1px solid var(--b1)', borderRadius: 10, padding: '1px 8px' }}>{groupUsers.length}</span>
                    </div>
                            ...
                            <tr key={user.id} style={{ borderBottom: i < groupUsers.length - 1 ? '1px solid var(--b1)' : 'none' }}>
                              <td style={{ ...tdBase, color: 'var(--text)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                                {user.id === brandUser?.id && (
                                  <span style={{ fontFamily: 'var(--sans)', fontSize: 11, background: 'rgba(242,205,26,0.12)', color: '#F2CD1A', border: '1px solid rgba(242,205,26,0.25)', borderRadius: 3, padding: '2px 7px', letterSpacing: '.06em', fontWeight: 500 }}>you</span>
                                )}
                                {user.name}
                              </td>
                              ...
                              <td style={tdBase}>
                                <select
                                  ...
                                  style={{ background: 'var(--s2)', border: '1px solid var(--b2)', borderRadius: 4, padding: '4px 8px', fontSize: 11, color: 'var(--t2)', fontFamily: 'var(--sans)', outline: 'none', opacity: (user.id === brandUser?.id || saving === user.id) ? 0.4 : 1 }}
                                >
                              ...
                              <td style={tdBase}>
                                <select
                                  ...
                                  style={{ background: 'var(--s2)', border: '1px solid var(--b2)', borderRadius: 4, padding: '4px 8px', fontSize: 11, color: 'var(--t2)', fontFamily: 'var(--sans)', outline: 'none', opacity: saving === user.id ? 0.4 : 1 }}
                                >
...
function AgeingConfigSection({ session }) {
  ...
  if (loading) return <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--t3)' }}>Loading...</p>;
  if (loadError) return <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: '#DE2A2A' }}>{loadError}</p>;
  if (config.length === 0) return <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--t3)' }}>No config found.</p>;

  return (
    <div>
      {loading && <p style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--t3)' }}>Loading...</p>}
      {config.length > 0 && (
        <>
            ...
            <thead>
              <tr>
                {['Stage', 'Warning (hrs)', 'Critical (hrs)', 'Auto-close (hrs)'].map(h => (
                  <th key={h} style={{ textAlign: 'left', fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--t3)', letterSpacing: '.1em', textTransform: 'uppercase', padding: '6px 10px', borderBottom: '1px solid var(--b1)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {config.map(row => (
                <tr key={row.stage}>
                  <td style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--text)', padding: '8px 10px', borderBottom: '1px solid var(--b1)' }}>{row.label}</td>
                  {['warning_hours', 'critical_hours', 'auto_close_hours'].map(field => (
                    <td key={field} style={{ padding: '6px 10px', borderBottom: '1px solid var(--b1)' }}>
                      <input
                        ...
                        style={{ width: 70, background: 'var(--s2)', border: '1px solid var(--b2)', borderRadius: 4, padding: '4px 8px', color: 'var(--text)', fontFamily: 'var(--sans)', fontSize: 11, outline: 'none' }}
                      />
                    </td>
                  ))}
                </tr>
