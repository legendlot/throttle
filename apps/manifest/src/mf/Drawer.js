...
import { signedInr, kindTone, label } from './data.js';

function linkedRecord(ref, orders) {
  if (!ref) return null;
  const ord = (orders || []).find((o) => o.no === ref || o.invoiceNo === ref);
  if (ord) return { icon: Package, title: ref, sub: 'View order detail', screen: 'orderDetail', arg: ord.id };
  if (/^PMT-/.test(ref)) return { icon: CreditCard, title: ref, sub: 'View in Payments', screen: 'payments' };
  if (/^DD-/.test(ref)) return { icon: HandCoins, title: ref, sub: 'View draw-down', screen: 'drawdowns' };
  if (/^VWINV-|^SFINV-/.test(ref)) return { icon: FileText, title: ref, sub: 'View in Documents', screen: 'documents' };
  return null;
}
...
export function Drawer({ entry, orders, onClose, onNav }) {
  if (!entry) return null;
  const tone = kindTone(entry.kind);
  const c = entry.amt < 0 ? 'var(--red)' : 'var(--green)';
  const link = linkedRecord(entry.ref, orders);
  const LinkIcon = link?.icon;
  const postedBy = ['payment', 'opening_balance'].includes(entry.kind) ? 'LOT'
    : ['order_cost', 'commission', 'charge', 'commission_base'].includes(entry.kind) ? 'Solve Factory' : 'System';
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, maxWidth: '92vw', zIndex: 51,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '20px 22px', borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <div style={{ fontFamily: MONO, fontSize: 16, fontWeight: 600, color: 'var(--t1)' }}>{entry.ref || label(entry.kind)}</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
          ...
          {link && (
            <>
              ...
              <div className="mf-icobtn" onClick={() => onNav(link.screen, link.arg)}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', borderRadius: 10, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
