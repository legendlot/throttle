'use client';
/* ════════════════════════════════════════════════════════════
   DepotSidebar — Sidebar shell adopted from Redline's Pit Wall v2,
   driven by Depot's NAV_PRIMARY/NAV_SETUP. Dispatch-primary IA +
   collapsed Setup drawer. Collapse state persists in
   localStorage `dp-sb-collapsed`.
   ════════════════════════════════════════════════════════════ */
...
export function DepotSidebar({ onCmdK, userLabel = '', userRole = '', onLogout }) {
  ...
  useEffect(() => {
    try { setCollapsed(localStorage.getItem('dp-sb-collapsed') === '1'); } catch (_) {}
    setHydrated(true);
  }, []);
  useEffect(() => {
  ...
  const setCol = (v) => { setCollapsed(v); try { localStorage.setItem('dp-sb-collapsed', v ? '1' : '0'); } catch (_) {} };
  ...
  return (
    <aside className="dp-rail" style={{ width: W, flexShrink: 0, background: 'var(--bg-2)', overflow: 'hidden',
      ...
      <div style={{ height: 56, display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0,
        ...
        {!collapsed && <span className="font-display" style={{ fontSize: 15, fontWeight: 700, letterSpacing: '0.1em',
          color: 'var(--t1)', whiteSpace: 'nowrap' }}>DEPOT</span>}
      </div>
