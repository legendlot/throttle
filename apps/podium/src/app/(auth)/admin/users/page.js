  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={sub}>Assign a Podium role to each LOT user. Accounts are created in Garage — this only sets People &amp; Performance access. No role = self-only (own profile + own wins).</p>
      </div>
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.id} style={{ opacity: u.active === false ? 0.5 : 1 }}>
                    ...
                    <td style={td}>
                      {u.podium_role
                        ? <span style={badgeKey}>{u.podium_role}</span>
                        : <span style={{ fontSize: 11, color: 'var(--text-3)' }}>self-only (default)</span>}
                    </td>
                    <td style={td}>
                      <select value={u.podium_role || ''} disabled={savingId === u.id}
                              onChange={(e) => assign(u.id, e.target.value)} style={select}>
