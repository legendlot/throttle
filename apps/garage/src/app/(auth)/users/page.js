...
export default function UsersPage() {
  ...
  return (
    <div style={{ color: 'var(--t1)' }}>
      ...
      {/* The gate-phone panel used to sit here too. It now lives ONLY inside the Phones tab —
          two doors to one job is what let the two enrolment surfaces drift apart. */}
...
function UsersList({ users, roles, loading, canManage, onCreate, onEdit, onRefresh }) {
  ...
  // Find-a-person search (Afshaan, 2026-09-10 — 82 users and no way to look one up).
  // Matches name, email, role (both the raw id and the display label, so "Super Admin" and
  // "super_admin" both work), team and phone. Space-separated terms are ANDed, so
  // "akshay viewer" narrows rather than widening.
  const [q, setQ] = useState('');
  const shown = useMemo(() => {
    const terms = q.toLowerCase().split(/\s+/).filter(Boolean);
    if (!terms.length) return users;
    return users.filter((u) => {
      const hay = [u.full_name, u.email, u.role, roleLabel(u.role), u.team, u.phone]
  ...
  return (
    <div style={panelStyle}>
      <div style={panelHeaderStyle}>
        <span>All Users <span style={{ color: 'var(--t3)', marginLeft: 6, fontSize: 11 }}>
          ({q ? `${shown.length} of ${users.length}` : users.length})
        </span></span>
      ...
      <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
        <input
          ...
          placeholder="Search name, email, role, team or phone…"
