...
export function UtmMarketingNote() {
  return (
    <div className="tw-note" style={{ marginTop: 10 }}>
      Only <b>marketing</b> sends are tagged. Utility and transactional messages (order updates,
      shipping, OTPs) are deliberately left untagged so they don&apos;t pollute campaign
      attribution. Links to non-LOT domains (courier tracking, for example) are never rewritten.
    </div>
  );
}
