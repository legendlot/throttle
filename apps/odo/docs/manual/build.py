#!/usr/bin/env python3
"""
System Manual build pipeline (reusable across apps).

  python3 build.py            # build the PDF into ./<Title>-Operations-Manual.pdf
...
What it does:
  ...
  2. Generates role-badge CSS and (optionally) an accent override straight from
     manual.json, so a new manual needs NO theme editing for roles or brand colour.
  3. Assembles one self-contained HTML document (cover + TOC + part dividers + chapters).
     Chapters with a "file" load that fragment; the rest render a styled stub from "summary".
  4. Renders to PDF with headless Chrome (full CSS, web fonts, page breaks).
  5. Measures the real page each chapter lands on (hidden sentinels), rebuilds the TOC
     with page numbers, and re-renders.
  6. Stamps a footer (title - version - page N) on every page except the cover, and adds
     a clickable PDF outline (bookmarks) for parts and chapters.
...
def _ensure_venv():
    try:
        ...
        pass
    if os.environ.get("_MANUAL_VENV") == "1":
...
# ── role-badge palette (cycled in role order) ─────────────────────
# Each tuple is (background, text, border). Soft tinted backgrounds, AA-ish text.
ROLE_PALETTE = [
    ("oklch(0.96 0.03 150)", "#1a7a3b", "oklch(0.82 0.08 150)"),  # green
    ("oklch(0.96 0.03 264)", "#2138c0", "oklch(0.82 0.08 264)"),  # blue
    ("oklch(0.96 0.04 60)",  "#b45309", "oklch(0.84 0.09 65)"),   # amber
    ("oklch(0.96 0.03 27)",  "#c01f1f", "oklch(0.83 0.09 27)"),   # red
    ("oklch(0.96 0.03 300)", "#7c3aed", "oklch(0.82 0.08 300)"),  # purple
    ("oklch(0.95 0.04 195)", "#0e7490", "oklch(0.80 0.08 195)"),  # teal
]

def role_css(cfg):
    lines = []
    for i, key in enumerate(cfg.get("roles", {})):
        bg, fg, bd = ROLE_PALETTE[i % len(ROLE_PALETTE)]
        lines.append(f".role.{key} {{ background: {bg}; color: {fg}; border-color: {bd}; }}")
    return "\n".join(lines)
...
def accent_css(cfg):
    ...
    if not a:
        return ""
    a2 = cfg.get("accent_soft", a)
    return f":root {{ --accent-strong: {a}; --accent-soft: {a2}; }}"
...
def chapter_html(cfg, part, ch):
    ...
    if ch.get("file"):
        ...
        body = (
            ...
            f'<p>This chapter is part of the manual&rsquo;s outline and will be written out '
            f'in full in an upcoming revision.</p>'
...
def assemble(cfg, css, page_of):
    ...
    # theme css first, then generated overrides (accent + role badges) so they win
    style = css + "\n" + accent_css(cfg) + "\n" + role_css(cfg)
    return (
...
def finalize(rendered_pdf, out_pdf, cfg, page_of_ch, page_of_part):
    ...
    writer.add_metadata({
        ...
        "/Author": cfg["owner"],
        "/Subject": f'{cfg["title"]} self-serve manual v{cfg["version"]}',
    })
