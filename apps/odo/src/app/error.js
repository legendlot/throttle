...
export default function Error({ error, reset }) {
  ...
  return (
    <div style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 440, width: '100%', textAlign: 'center', background: 'var(--surface, #15171c)',
        border: '1px solid var(--border, #2a2d35)', borderRadius: 14, padding: '32px 28px',
        boxShadow: 'var(--card-shadow, 0 6px 24px rgba(0,0,0,.3))' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          <button onClick={() => reset()} style={{ fontFamily: 'var(--ui, system-ui)', fontSize: 13, fontWeight: 600,
            ...
            background: 'var(--accent, #f5c518)', color: '#1a1a1a' }}>Try again</button>
          <button onClick={() => { try { window.location.reload(); } catch {} }} style={{ fontFamily: 'var(--ui, system-ui)', fontSize: 13, fontWeight: 600,
            padding: '9px 18px', borderRadius: 9, cursor: 'pointer',
            background: 'transparent', color: 'var(--t1, #e8eaed)', border: '1px solid var(--border, #2a2d35)' }}>Reload</button>
        </div>
