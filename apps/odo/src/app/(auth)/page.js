...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    salesGet('getBootstrap', {}, session)
      .then(b => setChannels((b?.channels || []).map(c => ({ channel_id: c.channel_id || c.id, name: c.name, type: c.type }))))
      .catch(() => {});
  }, [session]);
  ...
  const variantBoard = useMemo(() => {
    const arr = Object.entries(cur.variant).map(([code, v]) => ({ code, label: v.label, gross: v.gross, units: v.units }))
      .sort((a, b) => (variantMetric === 'units' ? b.units - a.units : b.gross - a.gross)).slice(0, 12);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(380px,1fr))', gap: 14 }}>
          ...
          <div className="so-card">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div className="so-kpi-lbl" style={{ margin: 0 }}>Top variants</div>
              <div style={{ display: 'flex', gap: 4, background: 'var(--surface2)', borderRadius: 7, padding: 3 }}>
                {['gross', 'units'].map(m => (
