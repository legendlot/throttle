...
test('defaults when env is empty', () => {
  ...
  assert.equal(c.mode, 'surface');
});

test('env overrides parse correctly', () => {
  const c = loadConfig({ ORIGIN_PINCODE: '560001', CUTOFF_HHMM: '16:30', NON_WORKING_DAYS: '0,6', CACHE_TTL_HOURS: '24', SHIP_MODE: 'express' });
