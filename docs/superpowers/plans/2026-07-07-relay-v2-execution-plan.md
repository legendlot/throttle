...
### M8 — Analytics layer (the render over data we already capture) — DONE (S206, commsops `a847a13a` + relay `4a76cab1`)
...
**As-built (S206):** migration `0013_comms_analytics_rpcs` applied — all 5 RPCs (`campaign_stats`/`journey_funnel`/`sends_overview`/`deliverability_health`/`campaign_attribution`, jsonb-returning, `service_role` EXECUTE) + 2 indexes (`events_link_clicked_message_idx` partial on `link_clicked`, `messages_queued_at_idx`). **Key correctness call:** `messages.status` is last-event-wins (sent→delivered→opened→clicked, each webhook overwrites), so delivered/opened are counted via the monotonic `delivered_at`/`read_at` timestamps (never rolled back), clicks via the `link_clicked` events join (`properties->>message_id`), complaint via `provider_status='email.complained'`, terminal states (bounced/failed/skipped/suppressed) via `status`. Journey membership can't use `source` (it's `journey:<enrolment_id>`, not journey_id) so the funnel is driven off `enrolment_steps ⋈ enrolments`. Worker: 5 `relay_view`-gated GET passthroughs (`getSendsOverview`/`getDeliverabilityHealth`/`getCampaignStats`/`getCampaignAttribution`/`getJourneyFunnel`) — blanket `canView` already gates every GET. App: `/analytics` (window picker 7/30/90d, overview KPIs, sends-by-day bars, per-campaign table, deliverability panel) + campaign-detail Performance panel (polls while sending) + journeys Funnel panel (Pipeline + enrolment-status badges + per-step result breakdown). Verified vs live internal-test data (S187 56-staff broadcast → 56 sent/delivered/100%; funnel logic proven on a synthetic set since S178 enrolment data was cleaned). RPC latency trivial (<500ms). Build clean; both deployed. **Browser smoke (needs Google login) is Afshaan's.**
...
**As-built (S206) — everything except the `wrangler.toml`/secret wiring:** migrations `0014_comms_queue_failures` + `0015_comms_send_budget` applied. **Warm-up budget (LIVE):** `settings.daily_send_budget` (null=unlimited) + `send_counters` per-IST-day + atomic `consume_send_budget()` (single INSERT…ON CONFLICT…WHERE, verified `[true,true,false]` at cap=2); `gate.js` consumes it as the FINAL marketing check (never burns a unit on an otherwise-skipped send) → `reason:'budget_exhausted'` skip row; settings UI field (super-admin). Default null = no-op, so live already. **Atomic campaign claim (LIVE):** `startCampaign` conditional PATCH `status=in.(approved,scheduled)→sending` so scheduler + manual "Send now" can't double-fire (`already_claimed`). **cancelSchedule action + campaign scheduled-state UI (LIVE).** **Scheduler + DLQ + alerts (CODE DEPLOYED but DORMANT):** `scheduled()` → `runScheduled` (due-campaign sweep via `startCampaign('scheduler')` + `checkDeliverabilitySpike` — last-100 real sends, alert if >10% failed/bounced or any complaint, ≤1/hr via `settings.last_alert_at`); `queue()` gained a `batch.queue==='commsops-dlq'` branch (writes `queue_failures` + alerts, always acks); `src/alerts.js` `alert()` (fail-open, no-ops if `SLACK_WEBHOOK_ALERTS` unset). **WIRED + LIVE (S206, Afshaan-approved, commsops `bfb18af2`):** `wrangler.toml` now has `[triggers] crons=["*/5 * * * *"]` + a `commsops-dlq` `[[queues.consumers]]` + `dead_letter_queue="commsops-dlq"` on the broadcast consumer; `commsops-dlq` queue created; deploy confirms `schedule: */5 * * * *` + `Consumer for commsops-dlq`. First tick verified safe (0 due campaigns; 56/56 delivered → no spike). **Only remaining (optional, Afshaan): mint a Slack Incoming Webhook for #relay-alerts → `wrangler secret put SLACK_WEBHOOK_ALERTS`** — until set, `alert()` no-ops (fail-open); scheduler + DLQ fully work, DLQ still records `queue_failures` rows.
...
**3. Slack alerting:**
- New `src/alerts.js`: `alert(env, text)` → Slack incoming webhook (`SLACK_WEBHOOK_ALERTS` secret, `wrangler secret put` — ask Afshaan to mint one for #system-updates or a new #relay-alerts).
