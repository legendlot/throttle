...
- [ ] 3d. **Balance migration** (`fbu_stock_to_ledger_cutover_v1`) — snapshot `stock_ledger` first (`CREATE TABLE store.safety_stock_ledger_fbu_cutover_2026_06_26 AS SELECT * FROM store.stock_ledger;`). For each built-part code, set `stock_ledger.opening_stock` so `closing_stock` equals the **agreed physical truth** (default = the current `fbu_stock` balance per product+variant+color summed onto the Common built-part code; or the Piyush count sheet if provided). Because `closing = opening + received − issued + returned` is generated, compute `new_opening = target_closing − (received − issued + returned)` per code. Build the per-code numbers from `fbu_stock` (or the count sheet), write via a single `UPDATE … FROM (VALUES …)` batch. **Show the before/after per SKU for sign-off before committing.**
