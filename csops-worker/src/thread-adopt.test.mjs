...
//
// The guard worth defending forever is the LAST test: adoption must never relabel the thread
// it adopts. 8,977 of the 10,254 numberless threads are legacy `whatsapp`, so the thread the
// web bot adopts is usually a WhatsApp one, and writing `relay_web=true` onto it would break
// the positive marker that separates Relay web threads from the legacy Chatwoot web widget.
